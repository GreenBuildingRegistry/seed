...
class InventoryViewTests(DeleteModelsTestCase):
    ...
    def test_upload_with_measure_duplicates(self):
        ...
        result = json.loads(response.content)
        self.assertEqual(result['status'], 'success')
        expected_message = {'warnings': [
            'Measure category and name is not valid other_electric_motors_and_drives:replace_with_higher_efficiency',
            'Measure category and name is not valid other_hvac:install_demand_control_ventilation',
